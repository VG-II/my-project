...
# - нет запрещённых файлов (.env, .env.local, .qwen/settings.local.json)
...
if [ -f .qwen/settings.local.json ];    then red "FAIL: .qwen/settings.local.json"; FAIL=1; else green "OK: no settings.local.json"; fi
...
if [ "$BRAND_HITS" -le 8 ]; then
  green "OK: $BRAND_HITS упоминаний (в пределах разрешённого - README URL/footer + AUTOPILOT приветствие/CTA + QWEN.md приветствие + skill attribution)"
